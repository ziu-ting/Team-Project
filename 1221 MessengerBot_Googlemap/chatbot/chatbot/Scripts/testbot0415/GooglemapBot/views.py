...
# 將toolong檔案內文字轉換
# ?
toolongf_v = [line.strip().encode('utf-8') for line in open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\toolong.txt',encoding = 'utf8').readlines()]
idd=''
toiletlat=''
toiletlong=''
wifilat=''
            ...
            if 'attachments' in incoming_message['entry'][0]['messaging'][0]['message']:
                if 'payload' in incoming_message['entry'][0]['messaging'][0]['message']['attachments'][0]:
                    if 'coordinates' in incoming_message['entry'][0]['messaging'][0]['message']['attachments'][0]['payload']:
                        ...
                        wifilat=incoming_message['entry'][0]['messaging'][0]['message']['attachments'][0]['payload']['coordinates']['lat']
                        wifilong=incoming_message['entry'][0]['messaging'][0]['message']['attachments'][0]['payload']['coordinates']['long']
        print(toiletlong)
        print(toiletlat)
        print('-------------------------------------------------')
        #print(wifilat)
...
def check_dict(fbid, recevied_message):
    ...
    # 檢測是否為多輪回答
    if idd in idcheck:
        print("進iddcheck了進iddcheck了進iddcheck了進iddcheck了進iddcheck了進iddcheck了")
        #廁所(收到使用者位置後)
        for i1 in range(D1):
            if idcheck[idd].find(dict["toilet"][i1]) >= 0 and ans <= 0:
                toilet(fbid,post_message_url)
                ans += 1
    ...
    # 檢測有無查位置類別需求關鍵字（內容物為判斷地點位置）
        for i4 in range(D4):
            if sentence.find(dict["location"][i4]) >= 0 and ans <= 0:
                ...
                message_contents(fbid,sentence,2,'地址')
                print(idcheck)
                print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
                ans += 1
    ...
    # 未完成實際內容
        ...
        for i17 in range(D17):
            if sentence.find(dict["問題"][i17]) >= 0 and ans <= 0:
                dle = sentence.find(dict["問題"][i17])
                dle_long = len(dict["問題"][i17])
                dle2 = dle + dle_long
                sentence = sentence[:dle] + sentence[dle2:]
                # response_text = "以下是充電資訊"
    ...
    if stationtime=='' and endstation!='':
        ...
        for Kaohour in range(25):
            if recevied_message ==kaotime[Kaohour]:                            
                ...
                print(stationtime)
                with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\jsondata\時刻表\MRT_Kaohsiung.json', 'r', encoding="utf-8") as f:
    ...
    if thsr_time_depart=='' and thsr_end!='':
        ...
        for thsrhour in range(25):
            if recevied_message ==thsrtime[thsrhour]:                            
                ...
                with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\jsondata\時刻表\HSR_180227.json', 'r', encoding="utf-8") as f:
                    data = json.load(f)
                today = time.strftime("%A")
                thsra = len(data)
    ...
    if tra_end=='' and tra_start!='':
        ...
        for tra in range(tranumber):
            if recevied_message == trastation[tra]: 
                ...
                with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\jsondata\時刻表\Train_180223.json', 'r', encoding="utf-8") as f:
                    tradata = json.load(f)
...
def wifi(fbid, post_message_url):
    ...
    with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\jsondata\wifi.json', 'r', encoding="utf-8") as f:
        wifidata = json.load(f)
    gmaps = googlemaps.Client(key='AIzaSyBa-fjzE3tQFWlybQD_cFfSlMsdks4AvxQ')

    global toiletlong,toiletlat
    wifians = 0
    if toiletlat=='' and toiletlong=='':
        ...
        status = requests.post(post_message_url, headers = {"Content-Type": "application/json"}, data = response_msg)
        print(toiletlat,toiletlong)
        print('wiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwiwi')
    elif toiletlat!=''and toiletlong!='' :
        a = toiletlat
        b = toiletlong
        print(toiletlat,toiletlong)
        print('fifififififififififififififiififififififififififififififi')
        ...
        for i in range(long):
            if ((float(wifidata[i]['LATITUDE'])-a)**2+(float(wifidata[i]['LONGITUDE'])-b)**2) < 0.000020295025:    #0.00000901度 = 1公尺 500m
                ...
                wifians=wifians+1
                response_msg = json.dumps({"recipient":{"id":fbid},"message":{"text":wifidata[i]['NAME']+'，地址是'+wifidata[i]['ADDR']}})
                status = requests.post(post_message_url, headers = {"Content-Type": "application/json"}, data = response_msg)
        if wifians==0 :
...
def toilet(fbid, post_message_url):
    with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\jsondata\toilet.json', 'r', encoding="utf-8") as f:
...
def ATM(fbid, post_message_url):
    with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\jsondata\post_atm.json', 'r', encoding="utf-8") as f:
        atmdata = json.load(f)
    gmaps = googlemaps.Client(key='AIzaSyBa-fjzE3tQFWlybQD_cFfSlMsdks4AvxQ')
    global toiletlong,toiletlat
    atmans = 0
...
def show(fbid, post_message_url):
    with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0415\GooglemapBot\jsondata\演唱會.json', 'r', encoding="utf-8") as f:
