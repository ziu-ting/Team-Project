...
# 將toolong檔案內文字轉換
# ?
toolongf_v = [line.strip().encode('utf-8') for line in open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0130-20180310T060152Z-001\testbot0130\GooglemapBot\toolong.txt',encoding = 'utf8').readlines()]
idd=''
toiletlat=''
toiletlong=''
...
def check_dict(fbid, recevied_message):
    ...
    # 未完成實際內容
        ...
        for i1 in range(D1):
            if last_state.find(dict["toilet"][i1]) >= 0 and ans <= 0:
                with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0130-20180310T060152Z-001\testbot0130\GooglemapBot\jsondata\toilet.json', 'r', encoding="utf-8") as f:
    ...
    # 未完成實際內容
        for i13 in range(D13):
            print('到底')
            if last_state.find(dict["高捷"][i13]) >= 0 :
            ...
            # response_text = "您附近的廁所有以下這些"
                ...
                if stationtime=='' and endstation!='':
                    ...
                    print(stationtime)
                    with open(r'D:\Moyege\Work\@PROJECT\1221Google_map\chatbot\chatbot\Scripts\testbot0130-20180310T060152Z-001\testbot0130\GooglemapBot\jsondata\時刻表\MRT_Kaohsiung.json', 'r', encoding="utf-8") as f:
                                ...
                                #print(b)
                                    ...
                                    if today == ('Monday' or 'Tuesday' or 'Wednesday' or 'Thursday'):
                                        if Kaoi % 3 == 0:
                                            ...
                                            if 2 > Kaox >= 0:
                                                response_msg = json.dumps({"recipient":{"id":fbid},"message":{"text":'第'+ dataKaoMRT[Kaoi]['Timetables'][Kaoj]['Sequence']+'班車: '+ dataKaoMRT[Kaoi]['Timetables'][Kaoj]['ArrivalTime']}})
                                                status = requests.post(post_message_url, headers = {"Content-Type": "application/json"}, data = response_msg)
